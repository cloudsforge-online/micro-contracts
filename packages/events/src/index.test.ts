...
test('the registry is an enumerated inventory — every addition is deliberate', () => {
  assert.deepEqual([...TOPIC_NAMES].sort(), [
    ...
    // 41-agora.md: the public square, fourteen topics landing with the service that emits them.
    // Not one of them carries a post body, a whisper body, or an email address — the registry
    // block states that rule and `audit.ts` states which two an operator is accountable for.
    'agora.bar.created',
